...
public:
  ...
  float millisecond() const { return std::chrono::duration<double, std::milli>(_t1-_t0).count();}

  float second() const { return std::chrono::duration<double>(_t1-_t0).count();}
  
  uint64_t nb_cycle() const { return _c1-_c0;}
  
  double CPI(size_t N) const { return double(_c1-_c0)/N;}

  double IPC(size_t N) const { return N/double(_c1-_c0);}
  
  double Gflops(size_t N) const { return double(N)/second();}
